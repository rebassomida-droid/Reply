...
import { createAssistant, updateAssistant, linkPhoneNumber } from '../services/vapi.service';
...
router.put('/', async (req: AuthRequest, res: Response): Promise<void> => {
  const {
    ...
    first_message, end_call_message, vapi_assistant_id, vapi_phone_number_id,
  ...
  if (existing.rows.length === 0) {
    const result = await pool.query(
      `INSERT INTO agent_config (name, persona_prompt, transfer_number, transfer_trigger, language, voice_id, first_message, end_call_message, vapi_assistant_id, vapi_phone_number_id, company_id,
      ...
      [name, persona_prompt, transfer_number, transfer_trigger, language, voice_id, first_message, end_call_message,
       vapi_assistant_id || null, vapi_phone_number_id || null, cid || null,
    ...
    const vapiPhoneId = vapi_phone_number_id !== undefined ? vapi_phone_number_id : existing.rows[0].vapi_phone_number_id;
    const result = await pool.query(
      `UPDATE agent_config SET name=$1, persona_prompt=$2, transfer_number=$3, transfer_trigger=$4, language=$5, voice_id=$6, first_message=$7, end_call_message=$8, vapi_assistant_id=$9, vapi_phone_number_id=$10,
         voice_speed=$11, voice_stability=$12, voice_similarity_boost=$13, voice_style=$14, voice_style_degree=$15, voice_use_speaker_boost=$16, updated_at=NOW()
       WHERE id=$17 RETURNING *`,
      [name, persona_prompt, transfer_number, transfer_trigger, language, voice_id, first_message, end_call_message, vapiId, vapiPhoneId,
       voice_speed ?? 1.0, voice_stability ?? 0.5, voice_similarity_boost ?? 0.75,
       voice_style ?? 0.0, voice_style_degree ?? 1.0, voice_use_speaker_boost ?? true,
       existing.rows[0].id]
    );
...
router.post('/sync', async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    // Collega il numero di telefono all'assistente (se configurato)
    if (config.vapi_phone_number_id && config.vapi_assistant_id) {
      try {
        await linkPhoneNumber(config.vapi_phone_number_id, config.vapi_assistant_id);
      } catch (linkErr) {
        console.warn('[Vapi] linkPhoneNumber warning:', linkErr);
        // Non blocca il sync — il linking fallisce se il numero è già su un altro assistente
      }
    }

    res.json({ success: true, vapi_assistant_id: config.vapi_assistant_id, vapi_phone_number_id: config.vapi_phone_number_id, operators_synced: operators.length });
  } catch (err: unknown) {
